	...
	for(funcs_iter = funcs.begin(); funcs_iter != funcs.end(); funcs_iter++) {
		...
		for(blocks_iter = blocks.begin(); blocks_iter != blocks.end(); blocks_iter++) {
			...
			for(edges_iter = edges.begin(); edges_iter != edges.end(); edges_iter++) {
				...
				/*if ((edge->type() == RET) and (target_block->start() == (Address) -1))
				{
                    cout<<"testing"<<hex<<edge->src()->last() << " "<<target_block->start()<<endl;
				}*/
				...
				/*if (edge->type() == DIRECT){
					if (edge->interproc() )
					{
						tail_function.push_back(arms_fun);
						cout << "tail function "<<arms_fun->get_base_addr()<<endl;
				...
				if (!arms_target_block) {
					...
					if (!arms_fun->is_plt())
					{
                        ...
                        //cout << "indirect unresolved transfer at " << hex<< source_block->last()<<endl;
					}
...
DICFG::copy_edge_type(ArmsEdge *arms_edge, ParseAPI::Edge *edge, bool indirect)
{
	...
	switch(edge->type()) {
		case CALL:
			...
			//if (!indirect)
				//cout<<"direct call: <target address,source address>: "<< hex<<edge->trg()->start()<<" "<<hex<<edge->src()->last()<<endl;
			arms_edge->set_type(type);
			break;

		case COND_TAKEN:
			//cout<<"cond taken: <target address,source address>: "<< hex<<edge->trg()->start()<<" "<<hex<<edge->src()->last()<<endl;
			arms_edge->set_type(arms_cond_taken);
			break;

		case COND_NOT_TAKEN:
			//cout<<"cond not taken: <target address,source address>: "<< hex<<edge->trg()->last()<<" "<<hex<<edge->src()->last()<<endl;
		...
		case INDIRECT:
			...
			arms_edge->set_type(type);
			//if (type == arms_indirect_jmp)
				//cout<<"indirect jmp: <target address,source address>: "<< hex<<edge->trg()->start()<<" "<<hex<<edge->src()->last()<<endl;
			break;

		case DIRECT:
			//cout<<"direct jmp: <target address,source address>: "<< hex<<edge->trg()->start()<<" "<<hex<<edge->src()->last()<<endl;
			type = (edge->interproc()) ?  arms_inter_direct_jmp : arms_direct_jmp;
			arms_edge->set_type(type);
			/*if (type == arms_inter_direct_jmp)
			{
		...
		case CATCH:
			//cout<<"catch: <target address,source address>: "<< hex<<edge->trg()->start()<<" "<<hex<<edge->src()->last()<<endl;
			arms_edge->set_type(arms_catch);
			break;

		case CALL_FT:
			//cout<<"call_ft: <target address,source address>: "<< hex<<edge->trg()->start()<<" "<<hex<<edge->src()->last()<<endl;
			arms_edge->set_type(arms_call_ft);
			break;

		case RET:
			//cout<<"ret: <target address,source address>: "<< hex<<edge->trg()->start()<<" "<<hex<<edge->src()->last()<<endl;
...
try_ParseAPI(BPatch_addressSpace *handle, int index)
{
	...
	Symtab    *dyn_st = sts->getSymtabObject();
	std::vector<Region *>dret;

	dyn_st->getDataRegions(dret);
	std::vector<Region *>::iterator reg_it;

	for(reg_it = dret.begin(); reg_it != dret.end(); ++reg_it)
	{
		Region *r = *reg_it;
		std::string name = r->getRegionName();
		if (name == ".rodata")
		{
			std::cout<<".rodata"<<"\n";
			Offset addr = r->getMemOffset();\
			unsigned long size = r->getMemSize();

			std::cout<<"addr "<<hex<<addr<<"\n";
			std::cout<<"size "<<hex<<size<<"\n";
			std::vector<unsigned long> rodata;
			rodata.push_back(addr);
			rodata.push_back(size+addr);
			parse_cfg->dataInfo.push_back(rodata);
		}
		...
		if (name == ".bss")
		{
			...
			bss.push_back(addr);
			bss.push_back(size+addr);
			parse_cfg->dataInfo.push_back(bss);
		}
		...
		if (name == ".text")
		{
			...
			text.push_back(addr);
			text.push_back(size+addr);
			parse_cfg->dataInfo.push_back(text);
		}
	}
	#endif
	...
	for (mods_iter = mods->begin(); mods_iter != mods->end(); mods_iter++) {
		...
		for(; funcs_iter != funcs->end(); funcs_iter++) {
			//printf("function address %p \n", (Address)(*funcs_iter)->getBaseAddr());
