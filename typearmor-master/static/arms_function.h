...
//	std::set<ArmsFunction*> get_callers(void) { return callers; } 
	...
	//address_t get_ret_addr(void)  {return ret_addr;}
	...
	void set_argbitmap(uint16_t bitmap){reg_bitmap = bitmap;}
	uint16_t get_argbitmap(void) {return reg_bitmap;}

    std::map <ArmsBasicBlock *, int> get_icall_args(void);

	#ifdef STRATEGY_WIDTH
	std::map <ArmsBasicBlock *, std::vector<uint16_t>> get_icall_width(void);
	void set_argwidth(std::vector<uint16_t> argwidth) {arg_width = argwidth;}
	std::vector<uint16_t> get_argwidth(void) {return arg_width;}
	#endif

	#ifdef STATISTICS
	void set_readRDX(int readRDX){read_RDX= readRDX;}
	int  get_readRDX(void){return read_RDX;}

	void mark_variadic_over(void){variadic_over = true;}

	bool is_variadic_over(void) {return variadic_over;}
	
	void set_Fread64to32(std::vector<int> Fr64to32){Fread64to32 = Fr64to32;}
	std::vector<int>  get_Fread64to32(void){return Fread64to32;}
	
	std::map <ArmsBasicBlock *, std::vector<int>> get_icall_pointer(void);

	std::map <ArmsBasicBlock *, int> get_icall_wrapper(void);
	
	std::map <ArmsBasicBlock *, std::vector<int>> get_icall_imm(void);
	
	std::map <ArmsBasicBlock *, std::vector<int>> get_icall_13(void);

	std::map <ArmsBasicBlock *, std::vector<int>> get_icall_36(void);

	std::map <ArmsBasicBlock *, std::vector<int>> get_icall_xor(void);

	std::map <ArmsBasicBlock *, int> is_icall_bet(void);
	#endif


	string to_string(void); 

	//added by yanlin, to return the width for arguments
...
private:
	...
	//std::set<ArmsFunction*> tail_call;
	...
	#ifdef STATISTICS
	int read_RDX;
	std::vector<int> Fread64to32;
	bool variadic_over = false;
	#endif
