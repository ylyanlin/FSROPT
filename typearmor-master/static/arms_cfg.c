...
int
CFG::foreach_function_2(int (*callback)(ArmsFunction*, void*, std::vector<std::vector<unsigned long>>), void *arg,std::vector<std::vector<unsigned long>> data_info)
{
    ...
    for(iter = functions.begin(); iter != functions.end(); iter++) {
        func = iter->second;
        if((ret = callback(func, arg, data_info))) {
            return ret;
        }
    }
...
std::map<std::tuple<int, address_t,address_t>,std::vector<std::pair<address_t,address_t>>> CFG::print_cfg()
{
	std::map<address_t, ArmsBasicBlock*>::iterator iter;
	ArmsBasicBlock *bb, *target_bb;
	ArmsFunction *f;
	ArmsEdge *e;
	size_t i;
	/*std::vector<address_t> ret;
	std::vector<address_t> direct call;
	std::vector<address_t> jmp;*/
	std::map<address_t, ArmsFunction*>::iterator iter_f;
	...
	for(iter = start2bb.begin(); iter != start2bb.end(); iter++) {
		...
		for(i = 0; i < bb->outgoing_edge_count(); i++) {
			...
			target_bb =e->target();
			if (edge_type_is_return(e->type())){
				CFI[bb->get_last_insn_address()].push_back(target_bb->get_start_address());
				//all_CFG[make_pair(0,bb->get_last_insn_address())].push_back(target_bb->get_start_address());
				all_CFG[std::make_tuple(0,bb->get_start_address(),bb->get_last_insn_address())].push_back(make_pair(target_bb->get_start_address(),target_bb->get_last_insn_address()));

				//cout << "ret: "<<hex<<bb->get_last_insn_address() <<" " << target_bb->get_start_address()<<endl;
			}
			else if (edge_type_is_direct_call(e->type())){
				//cout << "direct call: "<<bb->get_last_insn_address() <<" " << target_bb->get_start_address()<<endl;
				CFI[bb->get_last_insn_address()].push_back(target_bb->get_start_address());
				//all_CFG[make_pair(1,bb->get_last_insn_address())].push_back(target_bb->get_start_address());
				all_CFG[std::make_tuple(1,bb->get_start_address(),bb->get_last_insn_address())].push_back(make_pair(target_bb->get_start_address(),target_bb->get_last_insn_address()));
			}
			else if (edge_type_is_cond_taken(e->type())){
				//cout << "cond taken: "<<bb->get_last_insn_address() <<" " << target_bb->get_start_address()<<endl;
				CFI[bb->get_last_insn_address()].push_back(target_bb->get_start_address());
				//all_CFG[make_pair(2,bb->get_last_insn_address())].push_back(target_bb->get_start_address());
				all_CFG[std::make_tuple(2,bb->get_start_address(),bb->get_last_insn_address())].push_back(make_pair(target_bb->get_start_address(),target_bb->get_last_insn_address()));
			}
			else if (edge_type_is_cond_not_taken(e->type())){
				//cout << "cond not taken: "<<bb->get_last_insn_address()<<" "  << target_bb->get_start_address()<<endl;
				CFI[bb->get_last_insn_address()].push_back(target_bb->get_start_address());
				//all_CFG[make_pair(2,bb->get_last_insn_address())].push_back(target_bb->get_start_address());
				all_CFG[std::make_tuple(3,bb->get_start_address(),bb->get_last_insn_address())].push_back(make_pair(target_bb->get_start_address(),target_bb->get_last_insn_address()));
			}
			/*else if (edge_type_is_indirect_call(e->type()))
				cout << "cindirect call: "<<bb->get_last_insn_address()<<" "  << target_bb->get_start_address()<<endl;*/
			else if (edge_type_is_direct_jmp(e->type())){
				//cout << "direct jmp: "<<bb->get_last_insn_address() <<" " << target_bb->get_start_address()<<endl;
				CFI[bb->get_last_insn_address()].push_back(target_bb->get_start_address());
				//all_CFG[make_pair(3,bb->get_last_insn_address())].push_back(target_bb->get_start_address());
				all_CFG[std::make_tuple(4,bb->get_start_address(),bb->get_last_insn_address())].push_back(make_pair(target_bb->get_start_address(),target_bb->get_last_insn_address()));
			}
			else if (edge_type_is_inter_direct_jmp(e->type())){
				CFI[bb->get_last_insn_address()].push_back(target_bb->get_start_address());
				//cout << "inter direct jmp: "<<bb->get_last_insn_address() <<" " << target_bb->get_start_address()<<endl;
				//all_CFG[make_pair(3,bb->get_last_insn_address())].push_back(target_bb->get_start_address());
				all_CFG[std::make_tuple(4,bb->get_start_address(),bb->get_last_insn_address())].push_back(make_pair(target_bb->get_start_address(),target_bb->get_last_insn_address()));
			}
			/*else if (edge_type_is_inter_indirect_jmp(e->type()))
				cout << "inter indirect jmp: "<<bb->get_last_insn_address() << target_bb->get_start_address()<<endl;*/
			else if (edge_type_is_indirect_jmp(e->type())){
				//cout << "indirect jmp: "<<bb->get_last_insn_address() <<" " << target_bb->get_start_address()<<endl;
				CFI[bb->get_last_insn_address()].push_back(target_bb->get_start_address());
				//all_CFG[make_pair(3,bb->get_last_insn_address())].push_back(target_bb->get_start_address());
				if (target_bb->get_last_insn_address() == target_bb->get_start_address() && target_bb->get_start_address() < 0x10000)
				{
					cout << "the target of this indirect jump cannot be decided"<<endl;
					for(iter_f = functions.begin(); iter_f != functions.end(); iter_f++)
					{
						f = iter_f->second;
						if (!f->is_plt()){
							...
							all_CFG[std::make_tuple(6,bb->get_start_address(),bb->get_last_insn_address())].push_back(make_pair(f->get_base_addr(),end_addr));
						}
					}
				}
				else
					all_CFG[std::make_tuple(5,bb->get_start_address(),bb->get_last_insn_address())].push_back(make_pair(target_bb->get_start_address(),target_bb->get_last_insn_address()));
			}
			//else if (edge_type_is_call(e->type()))

			else if (edge_type_is_inter_direct_jmp(e->type())){
				tail_blocks[bb->get_start_address()] = bb;
			}

		}
	}
	/*for (std::map<std::pair<int, address_t>,std::vector<address_t>>::iterator it = all_CFG.begin(); it != all_CFG.end(); ++it)
	{
		cout << (*it).first.first <<" "<<(*it).first.second<<" ";
		std::vector <address_t> inVect = (*it).second;
	...
	//print the information of control flow transfer
	//cout <<"*****"<<CFI.size()<<endl;
	/*for(std::map<address_t, std::vector<address_t> >::iterator ii=CFI.begin(); ii!=CFI.end(); ++ii){
		cout << (*ii).first << ": ";
		//cout <<"******"<<" "<<endl;
...
address_t CFG::is_tail_block(ArmsBasicBlock *bb){
