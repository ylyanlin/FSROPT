...
public:
	...
	//added by yanlin
	int foreach_function_2(int (*callback)(ArmsFunction*, void*,std::vector<std::vector<unsigned long>>), void *arg, std::vector<std::vector<unsigned long>> data_info);
	...
	//std::map<std::pair<int, address_t>,std::vector<address_t>> print_cfg();
	std::map<std::tuple<int, address_t,address_t>,std::vector<std::pair<address_t,address_t>>> print_cfg();
	//std::map<address_t, ArmsFunction*> functions;
	std::map<address_t, ArmsFunction *> get_tail_functions() {return tail_functions;}
	std::map<address_t, ArmsBasicBlock *> get_tail_blocks() {return tail_blocks;}

	address_t is_tail_block(ArmsBasicBlock *bb);
	std::map<address_t, ArmsFunction*> get_functions() {return functions;}
...
private:
	...
	std::map<address_t,ArmsFunction *> tail_functions;
	std::map<address_t,ArmsBasicBlock *> tail_blocks;

	//store information about control flow transfer
	std::map<address_t,std::vector<address_t>> CFI;
	//std::map<std::tuple<int, address_t,address_t>,std::pair<std::vector<address_t>,std::vector<address_t>>> all_CFG;
