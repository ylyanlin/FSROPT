...
set -x
sudo DYNINSTAPI_RT_LIB=$DI/pwd/lib/libdyninstAPI_RT.so LD_LIBRARY_PATH=$DI/pwd/lib:$LD_LIBRARY_PATH $DI_OPT -load=`pwd`/../bin/fcfi_pass.di -fcfi_pass -binfo=$BINFO -args `pwd`/$exe $* 2>ta.log
